...
function the_Address_Showrow($id = 0)
{
    ...
    if ($arr_postmeta) {
        ...
        if (isset($arr_postmeta['stars'][0]) && $arr_postmeta['stars'][0] !="Bitte auswählen" )
...
function the_Address($id = 0)
{
    ...
    if ($arr_postmeta) {
        ...
        echo " <p>";
	if(isset($arr_postmeta['stars'][0]) && $arr_postmeta['stars'][0] != "Bitte auswählen")
		$stars = $arr_postmeta['stars'][0];
	else
		$stars=""; 
        if (isset($arr_postmeta['name'][0]))
            echo "<b>" . $arr_postmeta['name'][0] ."".$stars. " </b><br />";
        //if(isset($arr_postmeta['stars'][0]))
        //   echo "<b> Kategorie: ".$arr_postmeta['stars'][0]." </b> <br/>";
...
function de_create_post_type()
{
    ...
    register_taxonomy_for_object_type( 'post_tag', 'address' );
}
...
function getLWord()
{
    global $wp_query, $query;
    $slug = basename(get_permalink());
    if(is_page_template("page-state.php")) // TODO: find better workaround / fix (cbw 14.07.2016)
    {
        $last = $slug;
    }
    else
    ...
    $last=str_replace(array('ae','oe','ue','ss','Ae','Oe','Ue'),array('ä','ö','ü','ß','Ä','Ö','Ü'),$last);
    return $last;
}
...
/*
    if(!is_paged())
    {
        ...
        $argsSticky = array_merge($args, $secArgs);
        $postsSticky = new WP_Query($argsSticky);
        $args['posts_per_page'] -=2;
        $posts = new WP_Query($args);
        $posts = array_merge($postsSticky,$posts)
    }else
    {
        $secArgs = array(
...
function os_do_loop(&$obj) {
    ...
    foreach($obj as $post)
    {
	  ...
	  //  else
	  //      $thumb_url = ">";
	  if(isPremium($post->ID))
	  	$thumb_url = 'data-icon="'.get_stylesheet_directory_uri().'/imgs/hotel_prem.png'.'">';
	  else
		$thumb_url ='data-icon="'.get_stylesheet_directory_uri().'/imgs/square-11.svg'.'">';
        ?>
        <div class="marker" data-lat="<?php echo $location[lat]; ?>" data-lng="<?php echo $location[lng]; ?>"
...
//-----------------------------------------------------------
function the_Attribute2($att, $needle = "", $comp = 'LIKE', $type = 'address', $ppp = 10, $paged = 1)
{      
